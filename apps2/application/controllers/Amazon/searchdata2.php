...
class Searchdata2 extends BP_Controller {
   ...
   $this->javascript = array('jqueryui/ui/jquery-ui-1.8.21.custom.js');
	...
	public function getPotencialSkuSelectWH() {
		...
		$SQL= " DECLARE @temp TABLE(SKU nvarchar(max))
				insert into @temp(SKU)(
		        SELECT DISTINCT CAST(COMP.[ProductCatalogID] AS NVARCHAR(MAX)) AS 'SKU'
				...
				SELECT DISTINCT CAST(PC.[ID] AS NVARCHAR(MAX)) AS 'SKU'
	      	...
	      	$data['psSelect'] = $this->create_dropdown($SQL,'SKU','SKU', $potSku,'');
	...
	public function getPotencialSkuSelectBL(){
		...
		$SQL= " SELECT DISTINCT CAST(COMP.[ProductCatalogID] AS NVARCHAR(MAX)) AS 'SKU'
				FROM [Inventory].[dbo].[Compatibility] AS COMP
				LEFT OUTER JOIN [Inventory].[dbo].[ProductCatalog] AS PC ON (COMP.[ProductCatalogID] = PC.[ID])

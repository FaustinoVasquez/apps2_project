...
class Ordersalesdata extends BP_Controller {
    ...
    function getData() {
        ...
        $datefrom1 = date("Ymd", strtotime($datefrom));
        $dateto1 = date("Ymd", strtotime($dateto));
        ...
        $where .= " and ( (o.OrderDate >= '{$datefrom}' AND o.OrderDate <= '{$dateto}') 
                   OR (CONVERT(NVARCHAR,trk.PickupDate,112) BETWEEN '{$datefrom1}' AND '{$dateto1}' )
                    AND od.Adjustment = '0' ) ";

        // $where .= " and ( (o.OrderDate >= '{$datefrom}' AND o.OrderDate <= '{$dateto}') OR (trk.PickupDate >= '{$datefrom1}' AND trk.PickupDate <= '{$dateto1}')  )
        //             AND od.Adjustment = '0' ";
       ...
       // $orderby = ' ORDER BY o.OrderDate ';
        ...
        $SQL = "WITH mytable AS ($select , ROW_NUMBER() OVER (ORDER BY {$sidx} {$sord}) AS RowNumber
               ...
               FROM mytable
                WHERE RowNumber BETWEEN {$start} AND {$finish} ORDER BY RowNumber {$sord}";
     ...
     //   print_r($query->result_array());
        ...
        foreach ($query->result_array() as $row) {
            ...
            $responce->rows[$i]['cell'] = array($row['OrderDate'],
                      $row['OrderNumber'],
                      $row['DateShipped'],
                      $row['CompanyName']= utf8_encode($row['CompanyName']),
                      $row['Name'] = utf8_encode($row['Name']),
                      $row['CartName'],
                      $row['SKU'],
                      $row['Product'] =utf8_encode($row['Product']),
                      $row['LineItem']=utf8_encode($row['LineItem']),
                      $row['PricePerUnit'],
                      $row['QuantityShipped'],

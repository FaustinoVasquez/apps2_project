...
class PhilipsSales extends BP_Controller {
	...
	function index() {
		...
		$data = array(
			...
			'from'    => '/Reports/philipssales',
			'caption' => 'Philips Sales',
			'export'  => 'PhilipsSales',
			'sort'    => 'desc',
			'baseUrl' => base_url(),
		);

		$data['colNames'] = "['BareBulb','SKUSold','Name','Last30DaysSold','Last60DaysSold','Last90DaysSold','QOH','vQOH','tQOH','FBAQty','FBAInboundQty','PendingFBANeeded','BackOrders']";
		...
		$data['colModel'] = "[
                ...
                {name:'Backorder',index:'Backorder', width:80, align:'center',formatter:formatLink, sorttype: 'int'},
	...
	function getData() {
		...
		$selectSlice = 'SELECT [BareBulb]
                            ...
                            ,[Backorder]';
		$from        = ' FROM';
		$table       = ' [Inventory].[dbo].[RPTV-Philips-Sales-Report] ';
		$where       = '';
		$wherefields = array('BareBulb', 'SKUSold', 'Name');
		...
		$SQL = "WITH mytable AS ($select , ROW_NUMBER() OVER (ORDER BY [BareBulb] $sord) AS RowNumber
                                FROM {$table}{$where})

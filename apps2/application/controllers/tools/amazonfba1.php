...
class Amazonfba extends BP_Controller {
    ...
    public function index() {
        $user = 0;
        if ($this->MUsers->isValidUser($this->session->userdata('userid'), 884200) == 1) {
            $user = TRUE;
        }
        ...
        switch ($data['selectedorder']) {
            case 0:
                ...
                {name:'MITSKU',index:'MITSKU', width:70, align:'left'}, 
                {name:'BulbSKU',index:'BulbSKU', width:70, editable:true},
            ...
            case 1:
                ...
                $data['sortorder'] = 'desc';
                $data['colNames'] = "['OrderID','Channel','MITSKU','MerchantSKU','ASIN', 'FNSKU','OrderQTY','OrderNotes','OrderDate', 'Action']";
                //Cargamos al arreglo los colmodels
                $data['colModel'] = "[
                {name:'OrderID',index:'OrderID', width:60, align:'center'},
                {name:'Channel',index:'Channel', width:60, align:'left'},
                {name:'MITSKU',index:'MITSKU', width:60, align:'center', editable:true},
                {name:'MerchantSKU',index:'MerchantSKU', width:80, align:'left'}, 
                {name:'ASIN',index:'ASIN', width:60, align:'center'},
                {name:'FNSKU',index:'FNSKU', width:60, align:'center'},
                {name:'OrderQTY',index:'OrderQTY', width:70, align:'center'},
                {name:'OrderNotes',index:'OrderNotes', width:200, align:'center'},
            ...
            case 2:
                ...
                $data['sortorder'] = 'desc';
                //Cargamos al arreglo los colnames
                $data['colNames'] = "['OrderID','Channel','MITSKU','ASIN','MerchantSKU','FNSKU','OrderQTY','OrderNotes', 'OrderDate','CompletedDate','TrackingNumber','Completed','Action']";
                //Cargamos al arreglo los colmodels
                $data['colModel'] = "[
                {name:'OrderID',index:'OrderID', width:60, align:'center'},
	...
	$FinalDateFixed = $this->MCommon->fixFinalDate($FinalDate, 1);
        ...
        $table = '[Inventory].[dbo].AmazonFBA a';
        $table1 = '[Inventory].[dbo].AmazonFBAOrders b';
        $table2 = '[Inventory].[dbo].ProductCatalog c';

        $where = '';
        $from = ' FROM ';
        $join = ' ('.$table.' LEFT JOIN '.$table2.' ON a.MITSKU=c.ID) RIGHT JOIN '.$table1.' ON a.MerchantSKU=b.MerchantSKU ';

        $select = "SELECT b.OrderID, a.Channel, a.MITSKU , a.MerchantSKU, a.ASIN, a.FNSKU, b.OrderQTY, b.OrderNotes, Cast(b.orderdate as date) as OrderDate, c.customfield04 AS BulbSKU";
        ...
        $wherefields = array('b.OrderID', 'a.Channel', 'a.MITSKU', 'a.MerchantSKU','a.ASIN', 'a.FNSKU', 'b.OrderQTY', 'b.OrderNotes', 'OrderDate', 'c.customfield04');

        $where .=$this->MCommon->concatAllWerefields($wherefields, $dataSearch);

        $where.= "  and (OrderDate between '{$InitialDateFixed}' and '{$FinalDateFixed}')";
	...
	$FinalDateFixed = $this->MCommon->fixFinalDate($FinalDate, 1);
        ...
        $table = '[Inventory].[dbo].AmazonFBAOrders a';
        $table1 = '[Inventory].[dbo].AmazonFBA b';

        $where = '';
        $from = ' FROM ';
        $join =  $table.' LEFT JOIN '.$table1.' ON a.MerchantSKU=b.MerchantSKU  ';
        ...
        $wherefields = array('a.OrderID', 'b.Channel', 'b.MITSKU', 'a.ASIN', 'b.MerchantSKU', 'b.FNSKU', 'a.OrderQTY', 'a.OrderNotes','a.OrderDate','a.CompletedDate','a.TrackingNumber', 'a.Completed');

        $where .=$this->MCommon->concatAllWerefields($wherefields, $dataSearch);

        $where.= "  and (a.CompletedDate between '{$InitialDateFixed}' and '{$FinalDateFixed}')";
        $where .= " and ((a.Completed)='Yes') ";
       ...
       // echo $SQL;
        ...
        foreach ($result as $row) {
              $responce->rows[$i]['cell'] = array($row['OrderID']= utf8_encode($row['OrderID']),
                ...
                $row['OrderNotes']= utf8_encode($row['OrderNotes']),
                $row['OrderDate']= utf8_encode($row['OrderDate']),
                $row['CompletedDate']= utf8_encode($row['CompletedDate']),
                $row['TrackingNumber']= utf8_encode($row['TrackingNumber']),
                $row['Completed']= utf8_encode($row['Completed']),
            );
            $i++;
        }
    ...
    function validateAsin() {
        $SQL = "select count(ASIN) as qty from [Inventory].[dbo].AmazonFBA where ASIN = '{$_POST['av']}'";
        $return = $this->MCommon->getOneRecord($SQL);
        echo $return['qty'];
    }
    ...
    function saveAsin() {
        $SQL = "insert into [Inventory].[dbo].AmazonFBA (MITSKU, Channel, MerchantSKU,ASIN,FNSKU,Title) 
            Values 
            ({$_POST['skusend']},'{$_POST['ChannelSend']}','{$_POST['MerchanSKU']}','{$_POST['Asin']}','{$_POST['FNSKU']}','{$_POST['Description']}')";
    ...
    function completeOrder() {
        ...
        $SQL = "update [Inventory].[dbo].AmazonFBAOrders set completeddate='{$today}',completed='Yes' where orderid={$_POST['order']}";

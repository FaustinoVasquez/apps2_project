...
<fieldset class="bluegradiant">
	...
	echo form_dropdown('channel', $channelsOptions, isset($channelselect->channel) ? $channelselect->channel : $this->input->post('channel'), $dropdown);
	echo form_dropdown('countryCode', $countryCodeOptions, isset($countrycodeselect->countryCode) ? $countrycodeselect->countryCode : $this->input->post('countryCode'), $dropdown);
	echo form_dropdown('category', $categoryOptions, isset($categoryselect->category) ? $categoryselect->category : $this->input->post('category'), $dropdown);
...
<style type="text/css" media="screen">
    .ui-icon.graph {
        width: 16px;
        height: 16px;
        background-image:url('<?= base_url() ?>images/toolbar/graph-icon.png');
    }
...
<script type="text/javascript">
	...
	jQuery(grid).jqGrid('navButtonAdd', '#' + jQuery(grid)[0].id + '_toppager_left', { // "#list_toppager_left"
		...
		$("#dialog").html('<object width="550" height="360"  data="<?= base_url() . "index.php/Grids/asinbyday"?>">');
		$("#dialog").dialog("open");
...
<div id="dialog" title="Captured ASINs ">

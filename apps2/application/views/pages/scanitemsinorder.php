...
<fieldset class="bluegradiant">
	...
	$inputom = array('name' => 'omuser', 'value' => $omuser, 'autocomplete' => 'on', 'maxlength' => '50', 'size' => '5', 'onclick' => "this.value=''");
        echo " OMUser:" . form_input($inputom);
	
	$inputompwd = array('name' => 'ompwd', 'value' => $ompwd, 'autocomplete' => 'on', 'maxlength' => '50', 'size' => '8', 'onclick' => "this.value=''", 'onchange' => "this.form.submit()",);
        echo " OMPwd:" . form_password($inputompwd);

	echo '   OM:'; if($OMCredentials ) {echo '<b style="color:blue">Active</b>';}else{ echo 'Inactive';}
...
<script type="text/javascript">
        ...
        myGrid.jqGrid({
           ...
           $("#"+subgrid_id).html("<div style='background-color:#363737;width:100%;'>\n\
               ...
               jQuery("#"+subgrid_table_id).jqGrid({ 
                   ...
                   datatype: "json",  
                   colNames: ['OrderNumber','SKU','ItemNumber','Product','QuantityOrdered','QuantityShipped','QuantityReturned'], 
               colModel: [ 
		    {name:'OrderNumber',index:'OrderNumber', width:100, align:'center'},
                    {name:'SKU',index:'SKU', width:100, align:'center'},
                    {name:'ItemNumber',index:'ItemNumber', width:100, align:'center', hidden:true},
                    {name:'Product',index:'Product', width:200, align:'left'},
                    {name:'QuantityOrdered',index:'QuantityOrdered', width:60, align:'center'},
                    {name:'QuantityShipped',index:'QuantityShipped', width:60, align:'center'},
                    {name:'QuantityReturned',index:'QuantityReturned', width:60, align:'center'},
               ], 
                   width:900,
                   rowNum:250, 
                   pager: pager_id, 
          ...
          jQuery("#"+subgrid_table_id).jqGrid('navGrid',"#"+pager_id,{cloneToTop:true,edit:false,add:false,del:false,search:false})
 
	 jQuery("#"+subgrid_table_id).jqGrid('navButtonAdd', '#' + jQuery("#"+subgrid_table_id)[0].id + '_toppager_left', { // "#list_toppager_left"
            id:'scan',
	    caption: "ItemsInfo",
	    title:"Scan Items", 
            buttonicon: "ui-icon-disk",
            onClickButton: function(){
		    var selRowId = jQuery("#"+subgrid_table_id).jqGrid('getGridParam','selrow');
		    var OrderNumber = jQuery("#"+subgrid_table_id).jqGrid('getCell', selRowId, 'OrderNumber');
		    var QtyOrd = jQuery("#"+subgrid_table_id).jqGrid('getCell', selRowId, 'QuantityOrdered');
	...
	$("#credentials").dialog({
	    ...
	    "Create": {
		text: "Create",
		id: "btnSendCredentials",
	        click: function(){
			var mydata= $("#validate").serialize();
			var request = $.ajax({
...
//Funciones para OMC
    function formatLink(cellvalue, options,rowData) {
        if (cellvalue!=null){    
            return "<a href=<?= base_url()  . 'index.php' .$formatLink?>"+'&on='+ cellvalue +'&csid='+ rowData[4] +'&from=2'+">" + cellvalue + "</a>";
        }
...
<div id="dialog" title="Scan Orders" style="font-size:30px">
    <form id="scan">
	    ...
	    <input type="text" name="myItem" id="myItem"  class="text ui-widget-content ui-corner-all" />	
    </form>
    <br /><br />
    <center>

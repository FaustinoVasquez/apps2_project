...
<div class="div-banner">
  <form action="demo_form.asp" id ='myForm'>
    <label class="lb-menu">Search: </label><span id='rBrandSelect'><select class="sel-controler" name ="myBrands" id ="myBrands"></select></span>
    <label class="lb-menu">Country:</label><span id='rCountryCodeSelect'><select class="sel-controler" name ="myCountryCode" id ="myCountryCode"></select></span>
    <label class="lb-menu">Status: </label><span id='rStatusAsinSelect'><select class="sel-controler" name ="myStatusAsin" id ="myStatusAsin"></select></span>
    <button class="btn-blue-black" onClick="location.href='<?php echo base_url() ?>/index.php/Catalog/prodcat'"><span class="btn-title"> MIT </span></button>
          <!--  ---------------- I D -------------------  -->
...
<center>
  <div id="div-content">
    ...
    <div class="col-2">
      ...
      <div style="text-align:right;"><button class="btn-gray" id="openMail"><span class="btn-title"> @ </span></button></div>
      <div id="block" class="block" style="display:none;">
        <hr>
        <input type="text" name="emailTo" id="emailTo" placeholder="Recipients" class="inp-mail" value="newproducts@mitechnologiesinc.com">
        <input type="text" name="subject" id="subject" placeholder="Subject" class="inp-mail">
        <textarea name="note" id="note" placeholder="Note" class="txt-note"></textarea>
        <button class="btn-blue" id="SendMail"><span class="btn-title"> Send Mail </span></button>
        <button class="btn-blue" id="closeMail"><span class="btn-title"> Close </span></button>
      </div>
    </div>
    <!--  -----------------------------------  -->
    <div class="clear"></div>
    ...
    <div class="col-2">
      ...
      <div><label>Map To SKU: </label><span id='lPotentialSku'></span><select name ="SkuWH" id ="SkuWH" class="sel-controler3"></select><select name ="SkuBL" id ="SkuBL" class="sel-controler3" hidden></select></div>
...
//CLick Principal Buttons
  ...
  $('#openMail').click(function(){
    ...
    document.getElementById('openMail').style.display="none";
  });

  $('#closeMail').click(function(){
    document.getElementById('block').style.display="none";
    document.getElementById('openMail').style.display="";
    clearMail();
  });
...
$('#SendMail').click(function(){
    ...
    var Note = $("#note").val();
    if(Note != '' && EmailTo != ''){
      $.xResponse('sendEmailMiTechnologiesInc','get','json',{ id:ID, note:Note, to:EmailTo, sbj:Subject});
      alert("Your message has been sent");document.getElementById('block').style.display="none";document.getElementById('openMail').style.display="";
      clearMail();
    }
    else{alert('The Recipient or Note is null')}
  });
...
function getBareOrHousing(lamp){
   document.getElementById('myPotSku').value = lamp[0] ; 
...
function initializeData1(flag){
  ...
  if (!jQuery.isEmptyObject(rawAsin)){
  
     addElementsToMainData(rawAsin);
    initializeForm2WH();

  }else{
    alert('No more Asins on this Brand to check')
  }
...
function clearMail(){
  document.getElementById('emailTo').value = 'newproducts@mitechnologiesinc.com';
